...
class coachadddata(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        tk.Label(right_frame, text="Add your athlete's data", font=(
            'Arial', 20)).grid(row=0, column=0, padx=5, pady=5)
        ...
        def new_win():
            ...
            tk.Label(
                new, text="Make sure you have your athlete's resting and max heart rates").pack(pady=0)
            ...
            def getExcel():
                try:
                    ...
                    vm1 = subplot1.plot(x, a, color='green', linestyle='dashed',
                                        linewidth=3, marker='x', markerfacecolor='blue', markersize=12)
                    vm2 = subplot1.plot(x, c, color='blue', linestyle='dashed',
                                        linewidth=3, marker='x', markerfacecolor='blue', markersize=12)
                    vm3 = subplot1.plot(x, d, color='red', linestyle='dashed',
                                        linewidth=3, marker='x', markerfacecolor='blue', markersize=12)

                    subplot1.legend([vm1[0], vm2[0], vm3[0]], [
                                    "Yujin", "Chris", "Trevor"], loc="upper right")
                    ...
                    canvas.create_text(
                        150, 50, text=f"Yujin's weekly Vo2Max: {a_mean}{text[0]}", fill="black")
                    canvas.create_text(
                        150, 75, text=f"Chris's weekly Vo2Max: {c_mean}{text[1]}", fill="black")
                    canvas.create_text(
                        150, 100, text=f"Trevor's weekly Vo2Max: {d_mean}{text[2]}", fill="black")
                    canvas.pack()

                except Exception as e:
                    tk.messagebox.showerror("error", "incorrect file type")

            tk.Button(new, text='Load File', command=getExcel, bg='grey',
                      fg='white', font=('helvetica', 12, 'bold')).pack(pady=5)
            tk.Button(new, text='Go Back', command=new.destroy, bg='grey',
                      fg='white',  font=('helvetica', 11, 'bold')).pack(pady=5)

        tk.Button(right_frame, text="Open", command=new_win).grid(
            row=1, column=0, padx=5, pady=5)

        # Create frames and labels in left_frame
        tk.Label(left_frame, text="Instructions:").grid(
            row=0, column=0, padx=5, pady=5)
        tk.Label(left_frame, text="Click open to proceed").grid(
            row=1, column=0, padx=5, pady=5)
